...
data_dir <- "~/Downloads" ##CHANGE PATH
...
H_comb <- rbind(H1,H2) %>% st_zm() %>% 
  ...
  mutate(Description="")# H1 and H2 are already multipolygons, but adding this line
...
aop_kmz_localfile <- glue::glue("{data_dir}/aop-flight-boundaries.kml") ##CHANGE PATH # Donal had to convert KMZ to KML in Google Earth because he was missing KMZ driver for st_read()
#aop_kmz_localfile <- glue::glue("{data_dir}/aop-flight-boundaries.kmz")
...
#NLCD Data 
nlcd_codes <- readr::read_csv("/Users/olearyd/Git/SESYNC-NEON-DATA/NLCD/nlcd_legend_2011.csv") ##CHANGE PATH
...
data_dir <- "~/Git/SESYNC-NEON-DATA/data/results/"  ##CHANGE PATH
