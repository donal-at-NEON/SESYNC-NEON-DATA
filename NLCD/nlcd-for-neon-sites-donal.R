...
aop_kmz_localfile <- glue::glue("{data_dir}/aop-flight-boundaries.kml")
aop_sf <- st_read(aop_kmz_localfile) %>% st_zm() %>% 
  mutate(Site = substr(Name, 1, 8), domain= substr(Name, 1, 3))
aop_sf <- distinct(aop_sf, .keep_all = T) # remove duplicates for co-located terrestrial/aquatic sites
...
get_nlcd_percents <- function(aop_site, dataset_type = "Land_Cover"){
  ...
  ## multiple ploygons! This throws an error
