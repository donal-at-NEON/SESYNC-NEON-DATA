...
get_nlcd_percents <- function(aop_site, dataset_type = "Land_Cover"){
  ...
  if(dataset_type == "Impervious"){
    ...
    hist_df %>% readr::write_csv(glue::glue("outputs/imperv_hist_data-{dataset_type}-{aop_site}-{nlcd_year}.csv"))
    ...
    filename <- glue::glue("outputs/impervious-{aop_site}-{nlcd_year}.png")
    png(filename)
    plot(nlcd_site, maxpixels=1e8, mar = c(1,1,1,1), mfrow = c(1,1))
...
f2_LC=purrr::safely(get_nlcd_percents, otherwise=print(aop_site), quiet=F)

purrr::walk(aop_sites, ~f2_LC(.x, dataset_type = "Land_Cover"))

f2_IMP=purrr::safely(get_nlcd_percents, otherwise=print(aop_site), quiet=F)
#purrr::walk(aop_noD18, ~f2_IMP(.x, dataset_type = "Impervious"))
